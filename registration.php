...
<?php
    if(isset($_POST['register'])) {
        ...
        if (!empty($username) && !empty($email) && !empty($password)) {
            ...
            $saltQuery = "SELECT salt FROM users";
            $saltQueryResult = mysqli_query($connection, $saltQuery);
            if (!$saltQueryResult) {
                die("Salt query failed " . mysqli_error($connection));
            }

            $row=mysqli_fetch_array($saltQueryResult);
            $salt=$row['salt'];
            $password=crypt($password,$salt);

            $addUserQuery= "INSERT into users(username,user_firstname,user_lastname,user_email,user_pwd,user_role)";
            $addUserQuery.="VALUES('{$username}','{$firstname}','{$lastname}','{$email}','{$password}','subscriber')";
